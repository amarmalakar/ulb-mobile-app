...
import { COMPLAINT_PHOTO_MAX_COUNT } from "@/features/complaints/constants";

import { prepareComplaintPhotoForUpload } from "./prepare-complaint-photo-for-upload";

const UPLOAD_PATH = "/user/media/upload";
...
async function uploadOnePhoto(
  ...
  const prepared = await prepareComplaintPhotoForUpload(localUri);
  ...
  const res = (await client.post(UPLOAD_PATH, formData, {
    ...
    // Default axios timeout is 10s — too short for multi‑MB uploads.
    timeout: 120_000,
 ...
 */
export async function uploadComplaintPhotos(
  client: AxiosInstance,
  accessToken: string,
  localUris: string[],
): Promise<string[]> {
  const locals = localUris.filter(isLocalPhotoUri).slice(0, COMPLAINT_PHOTO_MAX_COUNT);
  ...
  return [...existingKeys, ...keys].slice(0, COMPLAINT_PHOTO_MAX_COUNT);
}
